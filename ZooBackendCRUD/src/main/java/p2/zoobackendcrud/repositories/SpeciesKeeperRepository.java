...
public interface SpeciesKeeperRepository extends JpaRepository<SpeciesKeeper, Integer>{
    ...
    @Query("SELECT k FROM SpeciesKeeper k JOIN FETCH k.species WHERE k.keeper.id = :idEmployee")
    List<SpeciesKeeper> findByEmployeeId(@Param("idEmployee") Integer idEmployee);
    
    @Query("SELECT k FROM SpeciesKeeper k JOIN FETCH k.keeper WHERE k.species.id = :idSpecies")
